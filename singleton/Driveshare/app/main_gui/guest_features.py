...
def show_available_cars(root, app):
    ...
    tk.Label(app.main_frame, text="Search Available Cars", font=("Helvetica", 14)).pack(pady=10)

    # --- Search form fields ---
    tk.Label(app.main_frame, text="Location:").pack()
    location_entry = tk.Entry(app.main_frame)
    location_entry.pack()



    tk.Label(app.main_frame, text="Minimum Price ($):").pack()
    min_price_entry = tk.Entry(app.main_frame)
    min_price_entry.pack()

    tk.Label(app.main_frame, text="Maximum Price ($):").pack()
    max_price_entry = tk.Entry(app.main_frame)
    max_price_entry.pack()

    result_frame = tk.Frame(app.main_frame)
    ...
    def perform_search():
        ...
        filtered_cars = []
        for car in cars:
            matches_location = location in car.location.lower() if location else True
            matches_price = True
            if min_price:
                matches_price = matches_price and car.price_per_day >= float(min_price)
            if max_price:
                matches_price = matches_price and car.price_per_day <= float(max_price)
            
            if matches_location and matches_price:
                print(matches_location and matches_price)
                filtered_cars.append(car)

        if not filtered_cars:
            tk.Label(result_frame, text="No cars match your search criteria.").pack()
        else:
            for car in filtered_cars:
                tk.Label(result_frame, text=str(car)).pack()

    tk.Button(app.main_frame, text="Search", command=perform_search).pack(pady=5)
    ...
    tk.Button(app.main_frame, text="Back", command=go_back).pack(pady=10)
