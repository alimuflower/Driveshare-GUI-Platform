...
def view_inbox_gui(root, app):
    ...
    messages = app.messaging_service.get_messages_for_user(user.email)
    if not messages:
        tk.Label(app.main_frame, text="No messages found.").pack()
    else:
        for m in messages:
            tk.Label(app.main_frame, text=f"From: {m['sender']}").pack()
            tk.Label(app.main_frame, text=f"Message: {m['message']}", wraplength=400, justify="left").pack(pady=5)
...
def view_reviews_gui(root, app):
    ...
    if not reviews:
        ...
        for r in reviews:
            tk.Label(app.main_frame, text=f"From: {r['reviewer']}").pack()
